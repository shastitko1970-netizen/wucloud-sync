...
Снимки аккаунта (pack / zip): [Dashboard → Бэкапы ST](https://wuproj.com/dashboard?section=backups).

### Бэкап data (MVP 0.8.2)

Кнопка **«Бэкап data»** в панели мода собирает pack (персонажи + чаты + мета) через API SillyTavern и кладёт в облако как `st_user_backup`.  
Полный zip папки `data/…` с диска — загружается вручную на сайте во вкладке **Бэкапы ST** (не через браузерный мод).

Квоты free: ~200 MB / 3 снимка; с подпиской — больше (лимит числа снимков).
...
| **Чаты персонажа** | Все чаты **текущего** персонажа (только чаты) |
| **ВСЕ чаты** | Все чаты **всех** персонажей (только чаты; confirm + Стоп) |
